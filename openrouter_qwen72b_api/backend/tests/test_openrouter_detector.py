"""Unit tests for OpenRouter Qwen 2.5 VL 72B Instruct VLM Layout Detector."""
...
def test_openrouter_detector_initialization():
    detector = LayoutDetector(confidence_threshold=0.40, device="cpu")
    assert detector.model_id == "qwen/qwen-2.5-vl-72b-instruct"
    assert detector.api_key is not None
    assert detector.confidence_threshold == 0.40


def test_openrouter_detector_inference_on_synthetic_image():
